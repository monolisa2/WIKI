...
export function DocRow({ doc }: { doc: DocRowData }) {
  ...
  return (
    <li className="group">
      <Link
        href={`/docs/${doc.slug}`}
        className="grid gap-x-4 gap-y-1.5 px-5 py-4 transition-colors hover:bg-surface-2 sm:grid-cols-[3.75rem_minmax(0,1fr)_10rem] sm:items-start"
      >
        <div className="pt-0.5">
          <TypeBadge type={doc.doc_type} />
        </div>
        <div className="min-w-0">
          <p className="flex items-center gap-1.5 text-[17px] font-semibold leading-snug text-ink transition-colors group-hover:text-accent">
            {doc.is_pinned ? (
          ...
          {doc.summary ? <p className="mt-1 line-clamp-2 text-[15px] leading-relaxed text-ink-2">{doc.summary}</p> : null}
        ...
        <div className="text-[14px] sm:pt-0.5 sm:text-right">
          {doc.revised_date ? (
            <span className={isStale(doc.revised_date) ? "font-medium text-warn" : "text-ink-2"}>{formatDate(doc.revised_date)} 개정</span>
          ) : (
